...
{
	e_cs_PacketType etype = static_cast<e_cs_PacketType>(packet[1]);

	switch (etype) {
	case sc_PACKETTYPE::MovePlayer: {
	...
	case sc_PACKETTYPE::PutPlayer: {
		sc_packet_put_player* put_packet = reinterpret_cast<sc_packet_put_player*>(packet);
		if (INVALID_ID == g_clients[ci].id) g_clients[ci].id = ci;
		if (ci == put_packet->id) {
			g_clients[ci].x = put_packet->x;
			g_clients[ci].y = put_packet->y;
		}
	}break;
	case sc_PACKETTYPE::RemovePlayer: break;
	case sc_PACKETTYPE::NPCChat: break;
	case sc_PACKETTYPE::PlayerChat: break;
	case sc_PACKETTYPE::Login_Ok: break;
	case sc_PACKETTYPE::Login_Fail: break;
	case sc_PACKETTYPE::MoveNPC: break;
	case sc_PACKETTYPE::PutNPC: break;
	case sc_PACKETTYPE::RemoveNPC: break;
	case sc_PACKETTYPE::Logout: break;
	default: break;
	}
}
...
void Adjust_Number_Of_Client()
{
	...
	if (g_bIsHotspotTest == false) {
		//cout << "teleport" << "\n";
		cs_packet_teleport my_packet2;
		my_packet2.size = sizeof(cs_packet_teleport);
		my_packet2.type = e_cs_PacketType::Teleport;

		SendPacket(num_connections, &my_packet2);
	}
	else {
		cs_packet_teleport my_packet;
		my_packet.size = sizeof(cs_packet_teleport);
		my_packet.type = e_cs_PacketType::ForTest;

		SendPacket(num_connections, &my_packet);
	}
...
void Test_Thread()
{
	while (true) {
		...
		for (int i = 0; i < num_connections; ++i) {
			...
			switch (rand() % 4) {
			case 0: my_packet.type = Up; break;
			case 1: my_packet.type = Down; break;
			case 2: my_packet.type = Left; break;
			case 3: my_packet.type = Right; break;
			}
...
void InitializeNetwork()
{
	...
	if (mod == 2)
		g_bIsHotspotTest = true;
	else
		g_bIsHotspotTest = false;
