...
if (!(bob->attr & BOB_ATTR_VISIBLE))
   return(1);
	...
	D3DXVECTOR3 pos = D3DXVECTOR3((bob->x - g_left_x) * TILE_WIDTH + (float)(TILE_WIDTH / 2.f),
		(bob->y - g_top_y) * TILE_WIDTH + (float)(TILE_WIDTH / 2.f) + g_HeightWeight, 0.0);
	...
	//if (bob->message_time > GetTickCount() - 2000) 
		Draw_Text_D3D(bob->message,static_cast<int>(pos.x),static_cast<int>(pos.y),D3DCOLOR_ARGB(255,1,255,1));
