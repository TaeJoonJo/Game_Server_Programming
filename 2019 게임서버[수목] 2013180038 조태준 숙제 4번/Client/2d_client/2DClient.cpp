...
// 680 730
#define WINDOW_WIDTH    750   // size of window
...
void ProcessPacket(char *ptr)
{
	...
	case e_sc_PacketType::MovePlayer:
	{
		...
		if (other_id == g_myid) {
			g_left_x = my_packet->x - 5.f;
			g_top_y = my_packet->y - 5.f;
			player.x = my_packet->x;
			player.y = my_packet->y;
		}
	...
	for (int i = 0; i < 11; ++i)
	{
		for (int j = 0; j < 11; ++j)
		{
			...
			if (((tile_x % 7 == 0)) && ((tile_y % 7 == 0)))
			{
				...
				wsprintf(coorditext, L"%3d", tile_x + tile_y);
				Draw_Text_D3D(coorditext, TILE_WIDTH * i + 7, TILE_WIDTH * j + 7, D3DCOLOR_ARGB(255, 255, 0, 0));
			}
		}
	}
	g_pSprite->End();
