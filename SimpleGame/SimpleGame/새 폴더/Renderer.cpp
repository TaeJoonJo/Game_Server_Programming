...
void Renderer::DrawSolidRect(CGameObject* pRect)
{
	...
	float x = pRect->GetX();
	float y = pRect->GetY();
	float size = pRect->GetSize();
	float r = pRect->GetRed();
	float g = pRect->GetGreen();
	float b = pRect->GetBlue();
	float a = pRect->GetAlpha();
...
void Renderer::DrawTexturedRect(CGameObject* pRect, GLuint texID)
{
	float newX, newY;
